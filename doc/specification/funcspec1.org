...
**** int selectOnTop(Engine engine)
     Selects what engine to display on the top screen, the other
     engine will be displayed on the bottom screen.
**** int setupBackground(Engine engine, int layer, int tileWidth, int tileHeight)
     Layer is either 0, 1, 2, or 3. 0 is in front, 3 is in back. Tile
     width and tile height must be divisible by 8.
**** int loadTileSet(Engine engine, TileSetDefinition *def)
...
***** Technical note
      The graphics engine will need to keep track of free blocks of
      VRAM. The easiest method will probably be an array of bytes that
      indicates if a 8*8 block (64 bytes) has been allocated. In that
      case, the allocator for background VRAM will have to keep track
      of 1792 blocks and for sprite VRAM 2048 blocks. This could also
      be packed into bytes as bit values so that the 1792 blocks could
      be represented by 224 bytes and the 2048 blocks could be
      represented by 256 bytes. This will be a balance between size
      and speed, so the exact method will appear later on.
**** int unloadTileSet(Engine engine, u16 tileSet)
     Remove tileset graphics from VRAM.
**** int setTile(Engine engine, int layer, int x, int y, u16 tileSet, u16 tile)
     Sets a tile in the abstract tilemap.
**** int setMap(Engine engine, int layer, Tile *map)
...
**** int setMap(Engine engine, int layer, u16 tileSet, u16 *map)
     As above, but with the same tileset for all the tiles.
**** int loadSprite(Engine engine, SpriteDefinition *def)
...
**** int removeSpriteInstance(Engine engine, int sprite)
     Removes the sprite instance.
**** int selectAnimation(Engine engine, int sprite, int animation)
     Select which animation to play for the sprite instance.
**** int showSprite(Engine engine, int sprite, bool show)
     Decide if you should show or hide the sprite.
**** int moveSpriteAbs(Engine engine, int sprite, int x, int y)
     Move sprite to an absolute coordinate.
**** int moveSpriteRel(Engine engine, int sprite, int dx, int dy)
     Move sprite relative to current position.
**** int getFreeMem(Engine engine, Type type /* BG or SPRITE */)
     Returns how many bytes of graphics memory is left for either
     background graphics or sprite graphics.
**** int getLargestFreeBlock(Engine engine, int type)
...
**** int getAvailableSprites(Engine engine)
